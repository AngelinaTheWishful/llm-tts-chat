...
- **会话名称侧栏编辑**（章节九十五）：侧栏「会话」折叠栏新增「重命名会话」输入框 + 按钮，选中会话后输入新名即可重命名，会话列表即时更新并持久化（`name.txt`）
- **语音命名规范**（章节九十五）：音频文件内部存储直接改为 `audio/{角色名}_{会话名}_{合成时间}_v{应用版本}_m{消息版本}.wav`
  - 例：`暴行_暴行-1_20260807_153012_v1.3.7_m1.wav`
  - 应用版本号单一来源：新增 `modules/version.py` 的 `APP_VERSION` 常量（与 build_zip/README 同步）
  - 消息版本号由调用方指定：首次回复=1，重新生成=2+；AI 消息保存时写入 `character` 字段
  - 文件名非法字符清洗 + 长度截断；旧会话 `msg_N.wav` 不受影响（逐条引用，无需迁移）
  - 会话导出 zip 音频天然带新名（零改动）；音频播放器旁新增「⬇️ 下载当前语音」按钮
- **统计面板修复**（章节九十五）：点击「查看统计」即时显示统计面板（原 `stats_output` 隐藏且 handler 未设 `visible=True` 导致看似无反应）

### 测试

- pytest 全量 **220** 项（新增 4 项：音频命名格式/清洗/截断/无角色字段）
- e2e_live **60/60**（新增 B27 会话重命名、B28 统计面板可见、B29 下载语音 handler）
