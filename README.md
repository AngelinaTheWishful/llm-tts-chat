...
> **v1.3.6 已发布**（2026-08-07）· [GitHub Releases](https://github.com/AngelinatheMellowWish/llm-tts-chat/releases)
...
- **TTS 根路径自动探测**（v1.3.6）：GPT-SoVITS 根路径启动自动探测（配置优先 → 启动报告 → 同级只读扫描），成功自动写回 `config.json`；前端「重新探测」按钮一键全量刷新（重探测 + 重扫权重 + 重应用角色音色预设）；角色 JSON 不再落盘本机绝对路径
...
1. 运行 `install_deps.bat` 安装依赖（创建 venv）
2. 确保 GPT-SoVITS 与 `llm_tts_update` 位于**同一父目录**（一键启动会自动探测 `GPT-SoVITS*` 目录）
3. 运行 `go-llm-tts.bat` —— 自动启动 GPT-SoVITS TTS API（api_v2.py）+ 聊天应用（两个窗口）
4. 首次启动进入配置向导，填写 GPT-SoVITS 路径、TTS API 地址、LLM 配置

> 说明：`go-llm-tts.bat` 会自动探测同级 `GPT-SoVITS*` 目录并启动其 `api_v2.py`（9880 已在运行则跳过）；TTS 加载约 30~90 秒，期间 app 先打开。启动步骤与错误码见 `logs/startup_report_*.txt`。
...
- **TTS 无声音**：确认 GPT-SoVITS 已就绪，状态栏显示"🟢 TTS API 在线"；一键启动会自动拉起 api_v2.py
...
- **报错带错误码**：UI 错误提示形如 `[LLM-004] 没有可用的 LLM 提供商`，凭错误码可快速定位；`logs/error.log` 与 `logs/run_report_*.txt` 含完整步骤与错误码
